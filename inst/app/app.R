...
#contents
instructions_content <- box(HTML('<h1>', '<strong>', '<u>', "Explanations on How to Use the App", '</strong>', '</u>', '</h1>', 
...
about_content <- box(HTML('<h1>', '<strong>', '<u>', "About", '</strong>', '</u>',  '</h1>', 
...
ref_content <-  box(HTML('<h1>', '<strong>', '<u>', "References", '</strong>', '</u>', '</h1>', 
    ...
    covidplot <- filterdataall() %>%
      ...
      theme(panel.background = element_rect(fill = "grey16"),
            ...
            axis.line.y = element_line(color = "white",
                                       size = 0.6))
    event_register(covidplot, 'plotly_click')
    ggplotly(covidplot, source = "A")
  })
